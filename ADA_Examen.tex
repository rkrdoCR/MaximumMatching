...
\begin{figure}[!htb]
    ...
    \begin{scope}[node every node/.style=state]
		\node(e) [circle, fill=yellow] at (0, 5.10) {\small $e$};
		\node(f) at (0, 4.25) {\small $f$};
		\node(a) at (0, 3.40) {\small $a$};
		\node(i) at (0 ,2.55) {\small $i$};
		\node(b) at (0 ,1.70) {\small $b$};;
		\node(j) [circle, fill=yellow] at (0 ,0.85) {\small $j$};
    \end{scope}
		\path   (e) edge (f)
  	 	     	(f) edge (a)        
   			    (a) edge (i)
	...
	\captionsetup{belowskip=0pt}
            \captionsetup{justification=centering}
    		\caption{\footnotesize Grafo de niveles construido con BFS usando los vertices libres de $V$, que en esta iteracion corresponde a $E$ como fuente, se almacenan los vértices libres del sext nivel en $F=\{j\}$}
            \label{fig:weather_filter1}
        \end{subfigure} \\\\
        \begin{subfigure}[b]{.9\linewidth}
    ...
    \begin{scope}[node every node/.style=state]
		\node(e) at (0, 5.10) {\small $e$};
		\node(f) at (0, 4.25) {\small $f$};
		\node(a) at (0, 3.40) {\small $a$};
		\node(i) at (0 ,2.55) {\small $i$};
		\node(b) at (0 ,1.70) {\small $b$};
		\node(j) at (0 ,0.85) {\small $j$};
    \end{scope}
	...
	\captionsetup{belowskip=0pt}
            \captionsetup{justification=centering}
    		\caption{\footnotesize Se aplica DFS usando $F=\{j\}$ como fuente para obtener el camino de aumentacion, por cada nodo en $F$ se debe eliminar las aristas y los vértices que estas conectan, asi como vertices y aristas huerfanos. El camino de aumentacion obtenido en esta iteracion es $P\{a-f,a-i,b-i,b-j,e-f\}$}
            \label{fig:weather_filter2}
        \end{subfigure} 
    \end{minipage}
\qquad
...
\end{tikzpicture}
	\captionsetup{belowskip=0pt}
                \captionsetup{justification=centering}
    		\caption{\footnotesize Se realiza una diferencia simetrica entre lo que tiene $M$ y el camino de aumentacion obtenido $M=M \triangle P\{j-b,b-i,i-a,a-f,f-e\}$ obteniendo como resultado $M=\{a-f,b-i,c-g,d-h\}$.\\Las aristas incluidas en el apareamiento maximo se resaltan en rojo y los nodos libres se resaltan en amarillo.}
                \label{fig:weather_activity}
            \end{subfigure}
        \end{minipage}
\end{figure}
